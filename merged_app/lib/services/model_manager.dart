  ...
  Future<void> _downloadFile({
    ...
    if (await _isUsableFile(destinationPath)) {
      onProgress(ModelDownloadProgress(
        stage: stage, title: title, filename: filename,
        receivedBytes: estimatedBytes, totalBytes: estimatedBytes,
        stageProgress: 1, overallProgress: completedWeight + stageWeight,
      ));
      return;
    }
    ...
    try {
      ...
      await for (final chunk in response.stream) {
        ...
        onProgress(ModelDownloadProgress(
          ...
          overallProgress:
              (completedWeight + (stageWeight * stageProgress)).clamp(0.0, 1.0).toDouble(),
        ));
      }
      await sink.flush();
      await sink.close();
      ...
      onProgress(ModelDownloadProgress(
        stage: stage, title: title, filename: filename,
        receivedBytes: totalBytes, totalBytes: totalBytes,
        stageProgress: 1,
        overallProgress: (completedWeight + stageWeight).clamp(0.0, 1.0).toDouble(),
      ));
    } catch (_) {
      if (await partialFile.exists()) await partialFile.delete();
      rethrow;
