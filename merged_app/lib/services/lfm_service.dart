...
class LfmService {
  ...
  /// If the intent router detects OCR, uses ML Kit instead of the LLM.
  Stream<String> reply({
    ...
    if (routedPrompt == IntentRouter.mlKitOcrTrigger) {
      ...
      yield recognizedText.isEmpty ? 'No text found in the image.' : recognizedText;
      return;
    }

    // ── LFM2-VL inference branch ──
    final engine = _engine;
    if (engine == null) throw StateError('The model is not loaded.');

    if (imagePath != null) await _ensureProjectorLoaded();

    final messages = <LlamaChatMessage>[
      LlamaChatMessage.fromText(role: LlamaChatRole.system, text: _runtimeSystemPrompt),
      LlamaChatMessage.fromText(role: LlamaChatRole.system, text: routedPrompt),
    ...
    try {
      ...
      if (imagePath != null) await _unloadProjectorIfNeeded();
    }
  }
  ...
  Future<void> _unloadProjectorIfNeeded() async {
    final engine = _engine;
    if (engine == null || !_projectorLoaded) return;
    try {
  ...
  List<LlamaChatMessage> _historyToLlamaMessages(List<ChatMessage> history) {
    final stableMessages = history
        .where((m) => !m.isStreaming && (m.hasImage || m.text.trim().isNotEmpty))
        .toList(growable: false);
    final trimmed = stableMessages.length > _maxHistoryMessages
        ? stableMessages.sublist(stableMessages.length - _maxHistoryMessages)
        : stableMessages;
    return trimmed.map(_toLlamaMessage).toList(growable: false);
  }
  ...
  LlamaChatMessage _toLlamaMessage(ChatMessage message) {
    ...
    if (message.hasImage) {
      return LlamaChatMessage.withContent(role: role, content: [
        LlamaTextContent(message.text.trim().isEmpty
  ...
  LlamaChatMessage _buildUserMessage({
    ...
    if (imagePath != null && imagePath.isNotEmpty) {
      return LlamaChatMessage.withContent(role: LlamaChatRole.user, content: [
